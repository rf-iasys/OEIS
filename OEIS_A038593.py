...
Checks against OEIS A038593 online sequence if available.
...
def run(n_start: int, n_end: int, oeis_data: dict[int,int] | None = None,
    ...
    print("\n=== OEIS A038593 ===")
    print("Differences between positive cubes in 1, 2 or 3 ways: union of A014439, A014440 and A014441.\n")
        ...
        if oeis_data is not None:
            if value_to_report in oeis_data:
                ...
                if not first_oeis_found:
                    ...
                    initial_offset = idx - 1  # leading values not in OEIS
            else:
                print(f"[{idx:6d}] {value_to_report:6d} (Not in OEIS)")
                if first_oeis_found:
                    differences.append(value_to_report)
        else:
            print(f"[{idx:6d}] {value_to_report:6d}")  # offline, just show computed
...
def main():
    ...
    oeis_url = "https://oeis.org/A038593/b038593.txt"
    try:
