...
Checks against OEIS A050492 online sequence if available.
...
def run(n_start: int, n_end: int, oeis_data: dict[int, int] | None = None,
    ...
    print("\n=== OEIS A050492 ===")
    print("Thickened cube numbers: a(n) = n*(n^2 + (n-1)^2) + (n-1)*2*n*(n-1).\n")
    ...
    for x in sorted(max_y_per_x.keys()):
        ...
        idx += 1
        value_to_report = int(y/2) if use_y_values else x
...
def main():
    ...
    oeis_url = "https://oeis.org/A050492/b050492.txt"
    try:
