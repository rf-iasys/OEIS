...
Checks against OEIS A000217 online sequence if available.
...
def run(n_start: int, n_end: int, oeis_data: dict[int, int] | None = None,
    ...
    print("\n=== OEIS A000217 ===")
    print("Triangular numbers: a(n) = binomial(n+1,2) = n*(n+1)/2 = 0 + 1 + 2 + ... + n.\n")
    ...
    for x in sorted(max_y_per_x.keys()):
        ...
        idx += 1
        value_to_report = int(y/8) if use_y_values else x
...
def main():
    ...
    oeis_url = "https://oeis.org/A000217/b000217.txt"
    try:
