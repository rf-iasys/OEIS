...
Checks against OEIS A168022 online sequence if available.
...
def run(n_start: int, n_end: int, oeis_data: dict[int,int] | None = None,
        ...
        use_y_values: bool = False, primes_only: bool = False) -> None:

    max_y_per_x = compute_max_y(n_start, n_end, stop_at_n_end=stop_at_n_end)

    print("\n=== OEIS A168022 ===")
    print("Noncomposite numbers in the eastern ray of the Ulam spiral as oriented on the March 1964 cover of Scientific American.\n")
...
def main():
    ...
    oeis_url = "https://oeis.org/A168022/b168022.txt"
    try:
    ...
    run(n_start, n_end, oeis_data, stop_at_n_end=stop_at_n_end,
        stop_at_index=stop_at_index, use_y_values=use_y_values, primes_only=primes_only)
