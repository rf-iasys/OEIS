...
Checks against OEIS A131426 online sequence if available.
...
def run(n_start: int, n_end: int, oeis_data: dict[int, int] | None = None,
    ...
    for x in sorted(max_y_per_x.keys()):
        ...
        idx += 1
        value_to_report = y - 1 if use_y_values else x
...
def main():
    ...
    oeis_url = "https://oeis.org/A131426/b131426.txt"
    try:
