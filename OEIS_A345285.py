...
Checks against OEIS A345285 online sequence if available.
...
def run(n_start: int, n_end: int, oeis_data: dict[int,int] | None = None,
    ...
    print("\n=== OEIS A345285 ===")
    print("""Sides of primary squares of type 1 (A344331). A primary square of type 1 is the smallest square that can be tiled with squares of two different sides a < b,
so that the numbers of small and large squares are equal.\n""")
    ...
    if oeis_data is not None:
        if first_oeis_found:
            if initial_offset != 0:
                print(f"Initial offset before first OEIS match: {initial_offset+1} value(s)")
            else:
...
def main():
    ...
    # Flags
    stop_at_n_end = True
    stop_at_index = 0   # 0 = no stop by index
    use_y_values = False

    oeis_url = "https://oeis.org/A345285/b345285.txt"
    try:
