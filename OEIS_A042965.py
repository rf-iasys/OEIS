...
Checks against OEIS A042965 online sequence if available.
...
def run(n_start: int, n_end: int, oeis_data: dict[int,int] | None = None,
    ...
    print("\n=== OEIS A042965 ===")
    print("Nonnegative integers not congruent to 2 mod 4\n")
    ...
    for x in sorted(max_y_per_x.keys()):
        ...
        value_to_report = int(y/4) if use_y_values else int(x/4)
...
def main():
    ...
    # Flags
    stop_at_n_end = False
    stop_at_index = n_end //2
    use_y_values = False
    primes_only = False
    exclude_even = False

    oeis_url = "https://oeis.org/A042965/b042965.txt"
    try:
