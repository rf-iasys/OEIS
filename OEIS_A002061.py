...
Checks against OEIS A002061 online sequence if available.
...
- skip_odd_idx: if True, skip reporting values with odd index
"""
...
def run(n_start: int, n_end: int, oeis_data: dict[int,int] | None = None,
        ...
        use_y_values: bool = False, skip_odd_idx: bool = True) -> None:

    max_y_per_x = compute_max_y(n_start, n_end, stop_at_n_end=stop_at_n_end)

    print("\n=== OEIS A002061 ===")
    print("Central polygonal numbers: a(n) = n^2 - n + 1\n")
    ...
    for x in sorted(max_y_per_x.keys()):
        ...
        # Increment index only for reported values
        idx += 1

        # Skip odd index if requested
        if skip_odd_idx and idx % 2 == 1:
            continue
...
def main():
    ...
    use_y_values = True
    skip_odd_idx = True        # skip values whose printed index is odd

    oeis_url = "https://oeis.org/A002061/b002061.txt"
    try:
